...
? Generated at: 2026-04-07 21:05:31.857546
"""
...
ingest_transactions_df = spark.read.format("parquet").load("s3://bank-cc-datalake/raw/ingest_transactions")
print("? SOURCE: ingest_transactions")

# ? SOURCE: ingest_cardholders
ingest_cardholders_df = spark.read.format("parquet").load("s3://bank-cc-datalake/raw/ingest_cardholders")
print("? SOURCE: ingest_cardholders")

# ? SOURCE: ingest_merchants
ingest_merchants_df = spark.read.format("parquet").load("s3://bank-cc-datalake/raw/ingest_merchants")
print("? SOURCE: ingest_merchants")

# ? SOURCE: ingest_fx_rates
ingest_fx_rates_df = spark.read.format("parquet").load("s3://bank-cc-datalake/raw/ingest_fx_rates")
print("? SOURCE: ingest_fx_rates")

# ? SOURCE: ingest_limits
ingest_limits_df = spark.read.format("parquet").load("s3://bank-cc-datalake/raw/ingest_limits")
print("? SOURCE: ingest_limits")

# ? TRANSFORM: clean_transactions
clean_transactions_df = ingest_transactions_df.selectExpr("*")  # no XFR rule found
print("? TRANSFORM: clean_transactions")

# ? TRANSFORM: clean_cardholders
clean_cardholders_df = ingest_cardholders_df.selectExpr("*")  # no XFR rule found
...
agg_daily_spend_df = enrich_tx_limits_df.selectExpr("*")  # no XFR rule found
print("? TRANSFORM: agg_daily_spend")

# ? TRANSFORM: agg_merchant_volume
agg_merchant_volume_df = enrich_tx_limits_df.selectExpr("*")  # no XFR rule found
print("? TRANSFORM: agg_merchant_volume")

# ? TRANSFORM: risk_overlimit
risk_overlimit_df = enrich_tx_limits_df.selectExpr("*")  # no XFR rule found
print("? TRANSFORM: risk_overlimit")

# ? TRANSFORM: fraud_detection
fraud_detection_df = enrich_tx_limits_df.selectExpr("*")  # no XFR rule found
print("? TRANSFORM: fraud_detection")

# ? TRANSFORM: aml_screening
aml_screening_df = fraud_detection_df.selectExpr("*")  # no XFR rule found
print("? TRANSFORM: aml_screening")

# ? SINK: report_daily_summary
agg_daily_spend_df.write.mode("overwrite").format("parquet").save("s3://bank-cc-datalake/curated/report_daily_summary")
print("? SINK: report_daily_summary")

# ? SINK: report_fraud_alerts
fraud_detection_df.write.mode("overwrite").format("parquet").save("s3://bank-cc-datalake/curated/report_fraud_alerts")
print("? SINK: report_fraud_alerts")

# ? SINK: report_cardholder_statements
agg_daily_spend_df.write.mode("overwrite").format("parquet").save("s3://bank-cc-datalake/curated/report_cardholder_statements")
print("? SINK: report_cardholder_statements")

# ? SINK: report_regulatory
aml_screening_df.write.mode("overwrite").format("parquet").save("s3://bank-cc-datalake/curated/report_regulatory")
print("? SINK: report_regulatory")

# ? SINK: notify_completion
report_daily_summary_df.write.mode("overwrite").format("parquet").save("s3://bank-cc-datalake/curated/notify_completion")
